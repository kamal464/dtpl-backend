...
const SchemaTableAttr = require('../models/schematableattrModel'); // Assuming your Sequelize model is named SchemaTableAttr
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await SchemaTableAttr.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await SchemaTableAttr.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await SchemaTableAttr.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await SchemaTableAttr.findByPk(id);
    ...
    await SchemaTableAttr.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await SchemaTableAttr.findByPk(existingRecord.id);
...
exports.getListSchemaTableAttr = catchAsync(async (req, res) => {
  try {
    ...
    const list = await SchemaTableAttr.findAll({
      where: {
