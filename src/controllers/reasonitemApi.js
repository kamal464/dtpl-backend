...
const ReasonItem = require('../models/reasonitemModel'); // Assuming your Sequelize model is named ReasonItem
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await ReasonItem.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await ReasonItem.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await ReasonItem.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await ReasonItem.findByPk(id);
    ...
    await ReasonItem.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await ReasonItem.findByPk(existingRecord.id);
...
exports.getListReasonItem = catchAsync(async (req, res) => {
  try {
    ...
    const list = await ReasonItem.findAll({
      where: {
