...
const emppersonal = require('../models/emppersonalModel'); // Assuming your Sequelize model is named Emppersonal
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await emppersonal.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await emppersonal.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await emppersonal.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await emppersonal.findByPk(id);
    ...
    await emppersonal.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await emppersonal.findByPk(existingRecord.id);
...
exports.getListPersonal = catchAsync(async (req, res) => {
    try {
      ...
      const list = await emppersonal.findAll({
        where: {
