...
const Reason = require('../models/reasonModel'); // Assuming your Sequelize model is named Reason
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Reason.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await Reason.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Reason.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Reason.findByPk(id);
    ...
    const updatedRecord = await Reason.findByPk(existingRecord.id);
...
exports.getListReason = catchAsync(async (req, res) => {
  try {
    ...
    const list = await Reason.findAll({
      where: {
