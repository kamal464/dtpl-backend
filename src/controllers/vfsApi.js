...
const Vfs = require('../models/vfsModel'); // Assuming your Sequelize model is named Vfs
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Vfs.findByPk(id);
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Vfs.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Vfs.findByPk(id);
    ...
    await Vfs.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await Vfs.findByPk(existingRecord.id);
...
exports.getListVfs = catchAsync(async (req, res) => {
  try {
