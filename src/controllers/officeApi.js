...
const Office = require('../models/officeModel'); // Assuming your Sequelize model is named Office
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Office.findByPk(id);
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Office.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Office.findByPk(id);
    ...
    const updatedRecord = await Office.findByPk(existingRecord.id);
...
exports.getListOffice = catchAsync(async (req, res) => {
  try {
    ...
    const list = await Office.findAll({
      where: {
