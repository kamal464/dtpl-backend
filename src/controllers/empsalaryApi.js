...
const Empsalary = require('../models/empsalaryModel'); // Assuming your Sequelize model is named Empsalary
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Empsalary.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await Empsalary.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Empsalary.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Empsalary.findByPk(id);
    ...
    await Empsalary.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await Empsalary.findByPk(existingRecord.id);
...
exports.getListSalary = catchAsync(async (req, res) => {
  try {
    ...
    const list = await Empsalary.findAll({
      where: {
