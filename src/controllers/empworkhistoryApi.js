...
const Empworkhistory = require('../models/empworkhistoryModel'); // Assuming your Sequelize model is named Empworkhistory
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Empworkhistory.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await Empworkhistory.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Empworkhistory.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Empworkhistory.findByPk(id);
    ...
    await Empworkhistory.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await Empworkhistory.findByPk(existingRecord.id);
...
exports.getListWorkHistory = catchAsync(async (req, res) => {
  try {
    ...
    const list = await Empworkhistory.findAll({
      where: {
