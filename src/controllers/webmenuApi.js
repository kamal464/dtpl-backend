...
const Webmenu = require('../models/webmenuModel'); // Assuming your Sequelize model is named Webmenu
...
exports.getRecord = catchAsync(async (req, res) => {
  ...
  const row = await Webmenu.findByPk(id);
...
exports.getAllRecords = catchAsync(async (req, res) => {
  try {
    ...
    const rows = await Webmenu.findAll({
...
exports.addObject = catchAsync(async (req, res) => {
  try {
    ...
    console.log(timeNow);
    const newRecord = await Webmenu.create({
      ...payload,
      sid: timeNow,
      rss: timeNow,
      lct: timeNow,
...
exports.updateObject = catchAsync(async (req, res) => {
  try {
    ...
    const existingRecord = await Webmenu.findByPk(id);
    ...
    await Webmenu.update(updateData, {
      where: { id: existingRecord.id },
    });

    const updatedRecord = await Webmenu.findByPk(existingRecord.id);
...
exports.getListWebmenu = catchAsync(async (req, res) => {
  try {
    ...
    const list = await Webmenu.findAll({
      where: {
